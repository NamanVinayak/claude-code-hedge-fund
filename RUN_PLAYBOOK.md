...
> **Model:** Use `model: "haiku"` for ALL persona/strategy agents in this step — they read structured facts and return structured JSON, so Haiku is sufficient and ~20x cheaper. The final decision agents (Step 4) use the default model.
...
> **Model:** Use `model: "haiku"` for the Head Trader agent — it synthesizes structured JSON signals into another structured output.
