...
def main():
    ...
    parser.add_argument("--asset-type", choices=("stock", "crypto"), default="stock",
                        help="Asset type: stock (default) or crypto")
    ...
    for ticker in tickers:
        print(f"Fetching data for {ticker}..." + (f" [crypto]" if asset_type == "crypto" else ""))

        if asset_type == "crypto":
            # Crypto: skip financial metrics, line items, insider trades (don't exist for crypto)
            metrics = []
            line_items = []
            insider_trades = []
            ...
            try:
                ...
                company_news = []  # Finnhub may not have crypto news
