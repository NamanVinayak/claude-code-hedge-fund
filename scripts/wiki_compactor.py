...
_LESSON_BULLET_RE = re.compile(
    r"^(\d{4}-\d{2}-\d{2})\s*\|\s*([A-Z][A-Z0-9.\-]*)\s*\|\s*([^|]+?)\s*\|\s*([+\-]?\$?[\d,.]+)\s*\|"
)


def aggregate_lesson_patterns(
    lessons_path: Path, today: date, dry_run: bool, lookback_days: int = 30
) -> int:
    """Synthesize a "## Patterns" section above "## Lessons" by grouping the
    last N days of bullets by SETUP TYPE and counting wins / losses / total
    P&L. Replaces any prior auto-generated Patterns block. Pure deterministic
    parsing — no LLM call.

    Returns the number of distinct setup-types aggregated (0 if no lessons
    ...
    # Each list entry: (outcome_str, signed_pnl_float)

    for line in text.splitlines():
        m = _LESSON_BULLET_RE.match(line.strip())
        if not m:
            continue
        date_s, _ticker, setup, outcome = m.group(1), m.group(2), m.group(3), m.group(4)
        try:
        ...
        if d < cutoff:
            continue
        # Parse outcome like "-$264.65" / "+$1,200" / "$50"
        ...
        if cleaned.startswith("-"):
            ...
            cleaned = cleaned[1:]
        try:
            amt = float(cleaned) * sign
        except ValueError:
            continue
        grouped[setup.strip()].append((outcome, amt))

    if not grouped:
        return 0

    rows = []
    for setup, entries in sorted(grouped.items(), key=lambda kv: -len(kv[1])):
        wins = sum(1 for _, amt in entries if amt > 0)
        n = len(entries)
        win_rate = (wins / n) * 100 if n else 0
        total_pnl = sum(amt for _, amt in entries)
        avg_pnl = total_pnl / n if n else 0
        def _fmt_money(amt: float) -> str:
            return f"{'-' if amt < 0 else '+'}${abs(amt):,.2f}"
        rows.append(
            f"| {setup} | {n} | {wins} | {win_rate:.0f}% | {_fmt_money(avg_pnl)} | {_fmt_money(total_pnl)} |"
        )

    new_block = (
        f"\n## Patterns (auto-generated by Sunday compactor, last {lookback_days} days)\n\n"
        f"| Setup Type | Trades | Wins | Win Rate | Avg P&L | Total P&L |\n"
        f"|---|---|---|---|---|---|\n"
        + "\n".join(rows)
        + "\n"
    )

    # Strip any prior Patterns block (from heading to next `## ` heading or EOF).
    cleaned_text, _n = re.subn(
    ...
    # Insert above "## Lessons" (or at end if section absent).
    if "## Lessons" in cleaned_text:
        final = cleaned_text.replace("## Lessons", new_block.lstrip("\n") + "\n## Lessons", 1)
    else:
        final = cleaned_text.rstrip() + "\n" + new_block

    if not dry_run:
        lessons_path.write_text(final)
    return len(grouped)
...
def run(dry_run: bool = False) -> None:
    ...
    # 6b. Aggregate lesson patterns (last 30 days, by setup type).
    lessons_path = WIKI_ROOT / "meta" / "lessons.md"
    pattern_groups = aggregate_lesson_patterns(lessons_path, today, dry_run)
    if pattern_groups:
        print(f"  Aggregated {pattern_groups} setup-type pattern(s) in lessons.md")
